...
=begin 
  affiche une répartition suivant le nombre de caractères, trie la liste suivant cette répartition
  et retourne une table de hachage (hash) qui illustre cette répartition  
=end 
def regroup_by_length(list)
  #répartir la liste suivant la taille, ce qui revient, juste, à la trier, suivant la taille. De plus, ceci est nécessaire pour la table de hachage, par la suite.
  ascending_length_sort(list)
  #listes pour créer une table de hachage 
  list_lengths = Array.new(list.size, 0) #liste des longueurs 
  list_number = Array.new(list.size, 0) #liste des occurences
  i = -1 #variable courante, commence de -1 car à la première entrée de la boucle sera 0
  #remplissage des deux listes
  list.each do |element| 
    if position_in_list(list_lengths, element.length) == -1
      i += 1
      list_lengths[i] = element.length	  
    end 
    list_number[i] += 1
  end 
  #creation du hash à partir des deux tableaux 
  hash_lengths_number = Hash[list_lengths.zip list_number]
  #affichage
  hash_lengths_number.each do |key, value|
    puts "Nombre de handle avec #{key} caractères:  #{value}"  
  end
  #retourner le hash 
  return hash_lengths_number
  #décommenter si besoin: return list  
end 
...
def perform() 
  key = -1
  while key != 9  
    # liste des logins des journalistes: si besoin, déclarer cette liste en dehors de la liste pour cumuler les changements effectués sur la liste
    ...
    case key
      ...
      when 6 #Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
        ...
        puts "Maintenant, le tableau est trié suivant un ordre croissant, selon la taille des handles:\n\n#{journalists_list}\n\n"
      ...
      when 8 #Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)
	    #test de la méthode regroup_by_length
        puts "Table de hachage pour ladite répartition: \n#{regroup_by_length(journalists_list)}\n\n" 
        puts "----------------------------------------------------------------------------------------------------------------------------------"      
...
=begin
  Programme Principal, qui appelle la méthode principale perform
=end
perform()
